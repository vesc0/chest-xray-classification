"""
Metadata parsing, multi-hot labels, patient-disjoint splits, and transforms.

Every split is patient-grouped and label-stratified, and training subsets are
nested prefixes, so a scaling curve measures data volume rather than the luck
of a draw. Val and test are fixed across all experiments.
"""
...
# The directory walk is slow on an external drive and several call sites need
# the index within one run.
_IMAGE_INDEX_CACHE: dict[Path, dict[str, Path]] = {}


def _build_image_index() -> dict[str, Path]:
    """
    Scan all image directories and return {filename: full_path}, cached.

    Raises if nothing is readable — usually an unmounted drive or an unset
    XRAY_DATASET_ROOT, which would otherwise surface as an empty DataFrame.
    """
    ...
    for img_dir in config.IMAGE_DIRS:
        if img_dir.exists():
            ...
            for path in img_dir.iterdir():
                # AppleDouble sidecars carry the .png suffix but are metadata
                # stubs, and would double the size of the index.
...
def _get_group_column(dataframe: pd.DataFrame) -> str | None:
    """The patient identifier column, if the frame carries one."""
...
def load_metadata() -> pd.DataFrame:
    """
    Read Data_Entry_2017.csv, adding 'labels', 'label_vector' and 'filepath'.
    """
    ...
    # A low match rate means the index and the CSV disagree, so fail rather
    # than silently training on whatever is left.
...
def _iterative_stratified_group_folds(
    ...
    Grouped iterative stratification (Sechidis et al., 2011): labels are placed
    rarest-first, so Hernia at 0.16% decides the assignment while there is still
    freedom to balance it. The 58% of studies with no positive label go last, on
    size alone, so the normal majority cannot skew any fold.
    ...
    def _pick(tied_folds: np.ndarray) -> int:
        ...
        Taking the lowest index front-loads rare labels into the early folds,
        biasing the nested subset prefixes.
        """
    ...
    for _ in range(config.NUM_CLASSES):
        ...
        for group_idx in _ordered_candidates(unassigned & (group_labels[:, label] > 0)):
            ...
            if best.size > 1:
                # Tie on this label: fall back to the emptiest fold.
...
def _order_shards_for_prefixes(
    ...
    Stratifying shards only balances them against each other, but a subset takes
    a *prefix*, so the order is what a 5k run sees. Greedily appends whichever
    shard keeps running prevalence closest to the pool's, scored relatively so a
    rare class weighs as much as a common one.
    """
    labels = _stack_label_vectors(dataframe)
...
def _stratified_subset(
    ...
    *,
    ...
    Smaller subsets are strict subsets of larger ones, which is what makes a
    5k/15k/30k curve measure data volume rather than which patients it caught.
    """
...
def _grouped_train_val_split(
    ...
    *,
    ...
    Applied to the full train_val pool before any subsetting, so validation —
    and therefore threshold calibration — is identical across every experiment.
    """
...
def _save_split_summary(
    ...
    """
    Record the class set and per-split support, so `--compare-all` can tell
    which runs are actually comparable.
    """
...
class ChestXrayDataset(Dataset):
    """Yields {'image': (3, H, W), 'label': multi-hot (NUM_CLASSES,), 'filename'}."""
...
# --- Transforms ---------------------------------------------------------------
# Bicubic matches the recipe four of the five backbones were pretrained under;
# torchvision's Resize defaults to bilinear, so leaving it implicit would
# fine-tune under a different filter than pretraining.
...
def _resize() -> transforms.Resize:
    ...
    Direct, not resize-shorter-side plus centre crop: the images are already
    square, and a crop would cut off the costophrenic angles and lung apices —
    exactly where effusions and pneumothoraces present.
    """
...
def get_train_transforms() -> transforms.Compose:
    """
    Mild augmentation approximating real positioning variation.

    No horizontal flip: chest radiographs have fixed laterality, so mirroring
    would teach the model that dextrocardia is unremarkable.
    """
...
def get_dataloaders() -> tuple[DataLoader, DataLoader, DataLoader]:
    """
    Build train, validation, and test loaders from the official NIH split.

    Test is always the full official split and validation is carved from the
    full train_val pool, so config.SUBSET_SIZE scales training data only.
    """
    ...
    # From the full pool, before subsetting: threshold calibration must see
    ...
    # One generator per loader, so one loader's draws cannot shift another's.
