...
The reasoning behind the choices in this pipeline: what is held fixed across
experiments, why each backbone and checkpoint was picked, what the explainability
and threshold machinery does, and what the test suite pins. See the
[README](../README.md) for setup and usage.
...
- [Supported models](#supported-models)
  - [Medical pretraining (`densenet121_xrv`)](#medical-pretraining-densenet121_xrv)
  - [Overriding the checkpoint (`--weight-tag`)](#overriding-the-checkpoint---weight-tag)
  - [Resolution (`--image-size`)](#resolution---image-size)
  - [Explainability per architecture](#explainability-per-architecture)
  - [Do the explanations explain the model?](#do-the-explanations-explain-the-model)
- [Tests](#tests)
...
[Preprocessing](#preprocessing). Support differs per model:
...
assigned to each validation and test image, and those rows are aligned across
runs by construction — the splits are fixed by `config.SEED`, never subset, and
read by an unshuffled loader — so averaging them needs neither the dataset nor a
GPU. `ensemble.py` runs in seconds:

```bash
# Select members by rule, then calibrate, score, and compare
python ensemble.py --auto

# See which runs the rule picks, and why each was kept or dropped
python ensemble.py --auto --dry-run

# Or name the members yourself
python ensemble.py --members s3_swin_384:swin_t s2_maxvit_full:maxvit_t
```

**Members are chosen by a rule, not by a search.** `--auto` takes the
best-scoring run per architecture family on validation, and drops any family
whose best run falls below `--val-floor` (default 0.75 macro AUROC). Greedy
forward selection over the same pool was measured here and scores about 0.0015
macro AUROC higher — inside the bootstrap interval on the ensemble's own margin,
bought by fitting the member list to 8,652 validation images and by selecting
the strongest run twice. The rule is reproducible and needs no such caveat.

The floor is a *did-it-converge* test, not a quality bar. A merely weak member
still contributes: the medically-pretrained DenseNet is the least accurate member
selected on this project's runs and the largest single contributor to macro
AUPRC, because its errors are the least correlated with the ImageNet backbones'.
A run that never converged is different in kind — near-uncorrelated with
everything because it is near-noise — and drags the mean down. Including every
run indiscriminately scores *below* the six the rule selects.

Members are equally weighted and probabilities averaged directly. `--rule logit`
(mean log-odds) lands within 0.0001 macro AUROC of the plain mean and
`--rule rank` about 0.001 below it, so the simplest is the default.

**What it produces.** The ensemble is written as its own experiment
(`outputs/s5_ensemble/` by default) rather than as another row beside an
architecture, because it is not an architecture and `compare_models()` reads its
table as one row per architecture. From there it behaves like any other run:
`--compare-all` lists it, and `threshold_analysis.py --experiment s5_ensemble
--model ensemble` reads it.

Because the margin an ensemble wins by is small, the run also bootstraps the
difference against its best single member — both scored on the *same* patient
resamples, so the shared variance from which images were drawn cancels:

```
[ensemble] Against the best single member (s3_swin_384:swin_t), on shared resamples:
  macro AUROC: delta +0.0055  95% CI [+0.0032, +0.0078]  favoured in 100.0% of draws
  macro AUPRC: delta +0.0185  95% CI [+0.0102, +0.0284]  favoured in 100.0% of draws
```

Read this the way the rest of the project's intervals are read: an interval
containing zero means the ensemble is not measurably better than its best member.
Note also that macro F1 at tuned thresholds barely moves (0.3288 to 0.3293) — the
gain is in ranking quality, which the threshold tuner spends on recall (0.457 to
0.483) rather than on F1. An ensemble is an AUROC/AUPRC result here, not an
operating-point one.

Two guards exist because both failures are silent. Members are checked to have
scored the same rows before anything is averaged: a run under a different `SEED`
or `VAL_SPLIT` produces arrays of identical shape whose rows are different
images, and the result would be a slightly worse number among plausible ones
rather than an error. And an ensemble is never offered as a member of another
ensemble — it writes predictions in the same format into the same tree, so a
second `--auto` run would otherwise select the first, make it its own baseline,
and report a margin of zero.
